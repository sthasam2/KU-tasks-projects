...
class Newtons_Divided_Difference:
    """
    A class used to represent a newton's divided difference question
    ...
    y : 2D list <float>
        list of functional values of x i.e. f(x) as well as divided differences [x0,x1],...
    ...
    def divided_difference_table(self):
        ...
        if self.check_input() == True:
            ...
            # Loop through x,y and calculate divided differences
            for i in range(1, n):
                for j in range(n - i):
        ...
        return "Error occurred during divide difference table calculation"
    ...
    def display_difference_table(self):
        ...
        # calculates up to which term divided difference and stores in a list
        header = []
        for i in range(1, n):
    ...
    def product_of_terms(self, i, value, x):
        ...
        ----------
        i : int
            term up to which product is to be carried out i.e. if i = 1, then (x-x0)*(x-x1) is calculated
        ...
        -------
        float
            product of differences for example (x-x0)*(x-x1)
        """
        pro = float(1)
        for j in range(i):
            pro = pro * (value - x[j])
...
def take_input():
    ...
    """
    instance = Newtons_Divided_Difference()
...
def take_multiple_input():
    ...
    if len(x) != n_val:
        print(
            "Error occurred while input. The number of x values does not correspond to the number of values to be inputted previously defined"
        )
        quit()
    ...
    if len(y) != len(x):
        print(
            "Error occurred while input. The number of x values does not correspond to f(x) i.e. y values"
        )
        quit()
    ...
    new_instance = Newtons_Divided_Difference(x, y_2D, n_val)
...
def demo():
    ...
    new_instance = Newtons_Divided_Difference(x, y, n)
    new_instance.display_difference_table()
    new_answer = new_instance.newton_interpolation_formula()
    print(
        f"\nThe interpolated value corresponding to {new_instance.value} is {new_answer}"
