        ...
        for i, I in enumerate(self.C):
            for head, bodies in I.items():
                for body in bodies:
                    ...
                    elif body[-1] == "." and head != self.G_prime.start:  # CASE 2 a
    ...
    def print_LR_parser(self, results):
        ...
        for i, step in enumerate(results["step"][:-1], 1):
            print(
                "".join(
                    [
                        f"| {history[i]:{just}{max_len}} "
                        for history, just, max_len in zip(
                            results.values(), justs.values(), max_lens.values()
                        )
                    ]
                )
                + "|"
